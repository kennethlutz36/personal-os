(()=>{
  ...
  const FOCUS_STALE_MS=20*1000;
  const PARSER_SYNC_KEY='personal-os-icloud-parser-v4-synced';
  ...
  function calendarRelevant(){
    const r=String(state.route||'overview');
    return ['overview','tasks','calendar'].includes(r)||!!document.querySelector('#content .v2-task-board,#content .v2-week');
  }
  function parserNeedsSync(){
    try{return localStorage.getItem(PARSER_SYNC_KEY)!=='1'}catch{return true}
  }
  function markParserSynced(){
    try{localStorage.setItem(PARSER_SYNC_KEY,'1')}catch{}
  }
  function fmtLast(ts){
  ...
  async function refreshCalendarData(){
    if(window.PersonalOSLiveV30?.refresh){
      await window.PersonalOSLiveV30.refresh(['events','calendar_sources'],{render:true});
    }else if(typeof refreshFromSupabase==='function'){
  ...
  // iCloud has no push webhook here, so poll only while calendar-driven surfaces are visible.
  setTimeout(()=>{injectLiveBar();if(calendarRelevant())syncApple(parserNeedsSync(),false)},2500);
  setInterval(()=>{injectLiveBar();if(document.visibilityState==='visible'&&calendarRelevant())syncApple(false,false)},AUTO_MS);
  window.addEventListener('focus',()=>{
    injectLiveBar();
    if(!calendarRelevant())return;
    const last=latestSync();
    if(parserNeedsSync()||!last||Date.now()-last>FOCUS_STALE_MS)syncApple(true,false);
  });
  document.addEventListener('visibilitychange',()=>{
    if(document.visibilityState==='visible'&&calendarRelevant()){
      injectLiveBar();
      const last=latestSync();
      if(parserNeedsSync()||!last||Date.now()-last>FOCUS_STALE_MS)syncApple(true,false);
    }
